...
** Last update Mon Dec 13 12:33:22 2010 john mille
*/
...
static char	*fill_tab(char *dest, char *str, int i)
{
  int		j;
...
char		**get_redir_right_argv(char *str)
{
  ...
  k = 0;
  in = 0;
  array = my_xmalloc((get_redir_right_argc(str) + 1) * sizeof(*array));
  while (str[i])
    {
      if (char_ok(str[i]) == 1 && in == 0)
	{
	  in = 1;
	  array[k] = fill_tab(array[k], str, i);
	  k += 1;
	}
      else if (char_ok(str[i]) == 0 && in == 1)
	in = 0;
